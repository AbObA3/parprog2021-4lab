...
public class equation {
    ...
    public equation(int paralel) {
        executorService = Executors.newFixedThreadPool(paralel);

    }

    void part(double[][] t0, double[][] t1, int sizeX, int sizeY, double[] rTLeft, double[] rTRight, double dx,
    ...
    void consider(double[][] tInit, int nX, int nY, int iterations) throws InterruptedException {
        double dx = 1.0 / nX;
        double dy = 1.0 / nY;
        double dt = 0.00001;
        int i1 = 0;
        ...
        for (int s = 0; s <= iterations; ++s) {
            ...
            part(T0, T1, nX, nY, rTLeft, rTRight, dx, dy, dt);
            double[][] tmp;
            tmp = T0;
            T0 = T1;
            T1 = tmp;
